...
module BIST_CTRL 
	...
	(clk, rst, runLBIST, PbarS, rstOut, PRPG_En, SRSG_En, SISA_En, MISR_En, done, sigAddr, LFSR_internalRst);
	...
	output reg LFSR_internalRst;

	reg SC_cnt_rst , TC_cnt_rst, TR_cnt_rst, group_cnt_rst;
	reg SC_cnt_en , TC_cnt_en, TR_cnt_en, group_cnt_en;
	wire [7:0] SC_cnt_out , TC_cnt_out, TR_cnt_out, group_cnt_out;
	reg SC_eq, TC_eq, TR_eq, group_eq;
	reg readSig;
	...
	counter #( .n(8) ) TestCycle_Cnt_inst (
            .clk(clk), 
            .rst(TC_cnt_rst), 
            .en(TC_cnt_en),
            .out(TC_cnt_out)
	...
	counter #( .n(8) ) TestRound_Cnt_inst (
            .clk(clk), 
            .rst(TR_cnt_rst), 
            .en(TR_cnt_en),
            .out(TR_cnt_out)
	...
	// assign sigAddr = (done == 1'b1) ? TR_cnt_out : 8'b0000;
	assign group_eq = (group_cnt_out == group+1) ? 1'b1 : 1'b0;
	counter #( .n(8) ) sigAddr_Cnt_inst (
            .clk(clk), 
            .rst(group_cnt_rst), 
            .en(done),
            .out(sigAddr)
