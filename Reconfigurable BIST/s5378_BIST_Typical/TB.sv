...
real numOfFaults = 1894;

reg NbarE = 1'b1; // Normal FS vs. Enhanced FS

integer bitsToFlip [0:4] = {24, 29, 5, 30, 0};
...
//###################################################################################################
	...
	if (NbarE == 1'b0) begin
		logFile = $fopen("FS_log_Typical.txt", "w");
		sigFile = $fopen("signatures_Typical.txt", "w");
		...
		$fwrite(logFile,"===> Fault Simulation Starting ...\n");
...
//###################################################################################################
	...
	if (NbarE == 1'b1) begin
		logFile = $fopen("FS_log_Enhanced.txt", "w");
